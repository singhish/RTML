...
class MetricsAdapter:
    ...
    def rolling_mae(self, window_size):
        return self._rolling_error(window_size, strategy=mean_absolute_error)

    def rolling_rmse(self, window_size):
        return self._rolling_error(window_size, strategy=lambda x, y: np.sqrt(mean_squared_error(x, y)))
    ...
    def _rolling_error(self, window_size, strategy=mean_absolute_error):
        rolled = np.array([])
        for i in range(self._df.size):
            if i <= window_size or np.isnan(self._predicted[i - window_size]):
        ...
        return pd.Series(rolled, index=self._df.index)
